    ...
    public partial class ManageRotatorBase : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack) {
                ...
                var licensing = App.GetLicensing(Request.RawUrl);
                if (licensing.IsActivated) {
                    foreach (var lic in licensing.Licenses)
                        lblLicenseMessage.Text += licensing.Licenses[0].RegistrationCode + "<br />";
                }
                
                btnActivate.HRef = licensing.ActivateUrl;
                //btnUnlockTrial.HRef = licensing.UnlockTrialUrl;
        ...
        {
            return DnnSharp.DynamicRotator.Core.ColorExt.ColorToHexString(color);
        }
