    ...
    public partial class QuickStatusAndLink : System.Web.UI.UserControl, IRegCoreComponent
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack) {
                ...
                if (_isAdmin) {
                    if (act == null) {
                        SetStatus("This copy of {0} is not licensed. <a href='{1}'>Unlock 30 Days Trial</a> or <a href='{2}'>Activate for Production</a>...", _RegCoreApp.ProductName, _unlockTrialUrl, _activateUrl);
                    } else if (act.RegCode.IsTrial) {
                        if (_RegCoreClient.IsTrialExpired(_RegCoreApp.ProductCode, _RegCoreApp.Version, Request.Url.Host)) {
                            SetStatus("{0} trial expired! <a href='{1}'>Purchase License</a> or <a href='{2}'>Activate for Production</a>...", _RegCoreApp.ProductName, _RegCoreServer.GetBuyUrl(_RegCoreApp), _activateUrl);
                        } else {
                            int trialDays = act.RegCode.DaysLeft;
                            if (trialDays <= 7) {
                                SetStatus("{0} days left from {1} trial! <a href='{2}'>Purchase License</a> or <a href='{3}'>Activate for Production</a>...", trialDays, _RegCoreApp.ProductName, _RegCoreServer.GetBuyUrl(_RegCoreApp), _activateUrl);
                            }
                        }
                    }
                } else { // public
